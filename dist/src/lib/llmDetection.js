...
exports.runLlmDetection = runLlmDetection;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const logger_1 = __importDefault(require("../utils/logger"));
const llmDetection_constants_1 = require("./llmDetection.constants");
// ── Model config ──────────────────────────────────────────────
...
function getClient() {
    ...
    if (!key) {
        logger_1.default.warn('LLM detection disabled — ANTHROPIC_API_KEY is not configured');
        _clientInitFailed = true;
        return null;
    }
    try {
        _client = new sdk_1.default({ apiKey: key, timeout: REQUEST_TIMEOUT_MS });
        return _client;
    }
    catch (err) {
        logger_1.default.error({ err: err?.message }, 'Failed to initialize Anthropic client — LLM detection disabled');
        _clientInitFailed = true;
        return null;
    }
}
...
function buildSystemPrompt(postureDescription) {
    return [
        ...
        '(b) Missing or inadequate compensation disclosure — FTC Endorsement Guides require clear and conspicuous disclosure of material connections before the fold',
        '(c) Performance claims or guarantees — including personal trading results, portfolio performance, or promissory language implying guaranteed positive outcomes',
        '(d) Urgency or pressure tactics designed to induce immediate action — especially when combined with account opening or referral incentives',
        '(e) Misleading or unbalanced statements about risk or returns — including unqualified fee claims ("commission-free") without mentioning other costs',
        '(f) If disclosure IS present, note whether it appears early in the content or only at the end',
        '',
...
function parseClaudeResponse(rawText) {
    ...
    for (const item of parsed) {
        ...
        if (!llmDetection_constants_1.LLM_RULE_CODE_SET.has(ruleCode)) {
            logger_1.default.warn({ ruleCode }, 'LLM detection returned an unknown rule code — dropping finding');
            continue;
        }
        // Validate severity
        if (!validSeverities.has(severity)) {
            logger_1.default.warn({ severity, ruleCode }, 'LLM detection returned an invalid severity — dropping finding');
            continue;
        }
        // Matched phrase + explanation are required
        if (matchedPhrase.length === 0 || explanation.length === 0) {
            logger_1.default.warn({ ruleCode }, 'LLM detection returned a finding with empty matchedPhrase or explanation — dropping');
            continue;
        }
        // Clamp severity against the per-rule ceiling
        const clamped = (0, llmDetection_constants_1.clampLlmSeverity)(ruleCode, severity);
        findings.push({
            ruleCode: ruleCode,
            ruleName: llmDetection_constants_1.LLM_RULE_CODE_NAME[ruleCode],
            severity: clamped,
 ...
 * Never throws.
 */
async function runLlmDetection(input) {
    ...
    const systemPrompt = buildSystemPrompt((0, llmDetection_constants_1.describePosture)({
    ...
    try {
        ...
        try {
            ...
            logger_1.default.warn({ err: parseErr.message, rawResponseLength: rawResponse.length }, 'LLM detection — failed to parse Claude response');
            return {
                ...empty,
                latencyMs: Date.now() - started,
                rawResponse,
        ...
        logger_1.default.warn({ err: err?.message, status: err?.status }, 'LLM detection — Claude API call failed');
        return {
            ...empty,
            latencyMs: Date.now() - started,
...
//# sourceMappingURL=llmDetection.js.map