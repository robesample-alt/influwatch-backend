...
async function main() {
  ...
  // ─── Media Assets ─────────────────────────────────────────
  const assets = [
    { id: 'MA-001', tenantId: TENANT_ID, contentRecordId: 'CR-003', assetType: 'THUMBNAIL' as const, assetUrl: 'https://storage.fundurex.com/iw/CR-003/thumbnail.jpg', mimeType: 'image/jpeg', durationSeconds: null },
    { id: 'MA-002', tenantId: TENANT_ID, contentRecordId: 'CR-003', assetType: 'VIDEO_FILE' as const, assetUrl: 'https://storage.fundurex.com/iw/CR-003/video.mp4', mimeType: 'video/mp4', durationSeconds: 847 },
    { id: 'MA-003', tenantId: TENANT_ID, contentRecordId: 'CR-001', assetType: 'SCREENSHOT' as const, assetUrl: 'https://storage.fundurex.com/iw/CR-001/screenshot.png', mimeType: 'image/png', durationSeconds: null },
  ];
  ...
  const extraEvents = [
    { id: 'SEED-EVT-001', tenantId: TENANT_ID, contentRecordId: 'CR-001', eventType: ArchiveEventType.STATUS_CHANGED, eventNote: 'Status changed: CAPTURED → ESCALATED — Unauthorized performance claim detected', actorId: 'COMPLIANCE-01' },
    { id: 'SEED-EVT-002', tenantId: TENANT_ID, contentRecordId: 'CR-001', eventType: ArchiveEventType.ESCALATION_RAISED, eventNote: 'Escalated to legal review — post reached 84K impressions before flagged', actorId: 'COMPLIANCE-01' },
    { id: 'SEED-EVT-003', tenantId: TENANT_ID, contentRecordId: 'CR-002', eventType: ArchiveEventType.STATUS_CHANGED, eventNote: 'Status changed: CAPTURED → PENDING_REVIEW — Off-platform communication flagged', actorId: 'SYSTEM' },
    { id: 'SEED-EVT-004', tenantId: TENANT_ID, contentRecordId: 'CR-004', eventType: ArchiveEventType.REVIEW_STARTED, eventNote: 'Human review started — minor wording deviation', actorId: 'COMPLIANCE-02' },
    { id: 'SEED-EVT-005', tenantId: TENANT_ID, contentRecordId: 'CR-004', eventType: ArchiveEventType.REVIEW_COMPLETED, eventNote: 'Review complete. "High conviction opportunity" flagged as minor deviation. Ambassador correction confirmed.', actorId: 'COMPLIANCE-02' },
    { id: 'SEED-EVT-006', tenantId: TENANT_ID, contentRecordId: 'CR-005', eventType: ArchiveEventType.REVIEW_COMPLETED, eventNote: 'Post outside approved window (2:14am). Timing policy reminder sent. No material violation.', actorId: 'COMPLIANCE-01' },
  ];
