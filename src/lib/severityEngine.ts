 ...
 */
export function postureSeverityFloor(posture?: string | null): Severity {
  const p = (posture || '').toUpperCase();
  if (p === 'CRITICAL') return 'MEDIUM';
  return 'LOW';
}
...
export function applySeverityFloor(
  ...
  detectionSeverities: Severity[],
  posture?: string | null,
): Severity {
  const detectionFloor = detectionSeverities.reduce(
