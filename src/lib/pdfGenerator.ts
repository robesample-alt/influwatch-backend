...
export function generateEvidencePdf(input: EvidencePackageInput, output: Writable): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor('#555555').font('Helvetica').fontSize(8)
       .text('This supervision system addresses obligations under FINRA Rules 2210/3110, SEC Marketing Rule 206(4)-1 and 206(4)-7, SEC Regulation A Rule 255, Securities Act Section 17(a), SEC Regulation Crowdfunding Rules 204 and 402(a), FTC Endorsement Guides §255.5, and applicable securities regulations. Recent enforcement actions against firms with compensated promoter programs include Robinhood ($26M+, 2025), Webull ($1.6M, 2025), Moomoo ($750K, 2024), M1 Finance ($850K, 2024), Public.com ($350K, 2025), Wefunder ($1.4M, August 2025), Republic ($950K, October 2025), Unicoin Inc (SEC charged May 2025), Nate Inc (SEC charged April 2026, $42M raised through fraudulent stock sales), SEC microcap sweep (2025), SEC paid athlete endorsement action (November 2024 — $250K, RIA Marketing Rule violation), SEC ETF influencer case (April 2024 — sliding-scale fee arrangement undisclosed), SEC nine-firm RIA sweep (September 2024 — $1.24M combined, testimonials lacking required disclosures), and Titan Global Capital Management ($1M+, August 2023 — misleading performance claims).', 50, y, { width: doc.page.width - 100, lineBreak: true });
    y += 80;
    ...
    for (const rec of input.records) {
      ...
      const grouped = groupDetections(rec.detections || []);
      if (grouped.length > 0) {
        doc.fillColor('#000000').font('Helvetica-Bold').fontSize(10).text('Compliance Findings', 50, y);
        y += 14;
        for (const g of grouped) {
          if (y > doc.page.height - 140) { doc.addPage(); drawHeader(doc, 'CONTENT RECORD · ' + rec.id + ' (cont.)'); y = 70; }
