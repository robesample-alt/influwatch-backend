...
export const SCENARIO_POOL: ContentScenario[] = [
 ...
 */
export function pickRandomScenarios(count: number, tenantType?: string): Array<{
  ambassadorId: string;
  sourcePlatform: string;
  contentType: string;
  campaignId?: string;
  ...
  if (tenantType && tenantType !== 'ALL') {
    ...
    pool = SCENARIO_POOL.filter(s => s.tenantType === tt || s.tenantType === 'ALL');
    // Fall back to full pool if filter yields too few
    if (pool.length < count) pool = SCENARIO_POOL;
  }
