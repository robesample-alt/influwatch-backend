...
import { getFindingCopy, type FindingCategory } from '../constants/findingCopy';
...
export const VALID_EXPOSURE_LEVELS: ReadonlySet<string> = new Set<ExposureLevel>([
  'NONE',
  'REVIEWER',
  'REVIEWER_PLUS_SUPERVISOR',
  'PRINCIPAL_EXCEPTION',
...
const REASON_LABEL: Record<ExposureReasonCode, string> = {
...
export interface ExposureInput {
  // Compensation context (from CompensationStructure Phase 1 fields)
  ...
  // Severity (already computed by the existing pipeline)
  ...
  // ISSUER tenants only — flag set when the content creator is an
  // officer, director, employee, or significant shareholder of the
  // issuer. Associated persons are exempt from broker-dealer
  // registration analysis when promoting their own offering.
  // Suppresses EXP-001 through EXP-004 from firing regardless of
  // compensation type. Anti-fraud and disclosure rules still apply.
  ...
  campaignViolationCount?:      number | null;   // TODO: campaign-level pattern
}
 ...
 */
export function computeExposure(input: ExposureInput): ExposureOutput {
  const reasons: ExposureReasonCode[] = [];
  let level: ExposureLevel = 'NONE';

  const compType  = ((input.compensationType || '') as string).toUpperCase();
  const txnClass  = ((input.transactionalityClass || '') as string).toUpperCase();
  const sev       = ((input.severity || 'LOW') as string).toUpperCase() as Severity;
  const tenant    = ((input.tenantType || '') as string).toUpperCase();
  ...
  for (const code of input.hitRuleCodes) {
    categories.add(getFindingCopy(code).category);
  }
  ...
  let escalateToPrincipal =
    hasExp013Trigger ||                                                       // EXP-013
...
function buildSummary(
  level: ExposureLevel,
  reasons: ExposureReasonCode[],
): string {
  if (level === 'NONE') return 'No elevated supervisory exposure identified.';

  const levelLabel: Record<ExposureLevel, string> = {
    NONE:                      'No exposure',
