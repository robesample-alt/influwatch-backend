...
import { transcribeUrl } from '../services/transcription.service';
        ...
        if (isVideo && mediaUrl) {
          try {
            ...
            if (transcript) {
              ...
              await attachMediaAsset(ambassador.tenantId, created.id, {
                assetType: 'TRANSCRIPT_FILE' as any,
                assetUrl: `transcript://${created.id}`,
                mimeType: 'text/plain',
              });
              logger.info({ recordId: created.id }, 'Video auto-transcribed via Whisper');
            }
          } catch (transcribeErr) {
            logger.warn({ recordId: created.id, err: transcribeErr }, 'Video transcription failed — record saved without transcript');
          }
        }
