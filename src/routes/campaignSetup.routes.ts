...
router.post('/:id/promoters', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await withTenantContext({ tenantId }, async (tx) => {
      ...
      if (!promoter) throw Object.assign(new Error('Promoter not found'), { status: 404 });
      ...
      if (!comp) throw Object.assign(new Error('Compensation structure not found'), { status: 404 });
...
router.get('/:id/promoters', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.user!.tenantId;
    const result = await withTenantContext({ tenantId }, async (tx) => {
...
router.delete('/:id/promoters/:pid', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.user!.tenantId;
    const result = await withTenantContext({ tenantId }, async (tx) => {
...
router.post('/:id/policy', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.user!.tenantId;
    const campaignId = req.params.id;
...
router.get('/:id/policy', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.user!.tenantId;
    const result = await withTenantContext({ tenantId }, async (tx) => {
      return tx.campaignPolicy.findFirst({ where: { campaignId: req.params.id, tenantId } });
    });
    if (!result) return res.status(404).json({ error: 'No policy found for this campaign' });
    return res.json(result);
  } catch (err) { next(err); }
});
...
router.post('/:id/activate',
  requireRole(InternalActorRole.REGISTERED_PRINCIPAL, InternalActorRole.DESIGNATED_SUPERVISOR),
...
router.get('/:id/setup-status', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.user!.tenantId;
    const campaignId = req.params.id;
