...
import { generateMagicLinkToken } from '../utils/promoterAuth';
import { sendPromoterInvite } from '../utils/mailer';
...
router.post('/send-invite', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.user!.tenantId;
    ...
    logger.info({ ambassadorId }, 'Portal invite sent by internal actor');
...
router.get('/portal-status/:ambassadorId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.user!.tenantId;
